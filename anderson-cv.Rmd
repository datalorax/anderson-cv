...
```{r}
kable(pubs_tbl[seq(13, nrow(pubs_tbl)), ])
```
...
```{r}
kable(conf_tbl[seq(26, nrow(conf_tbl)), ])
```
...
```{r software2}
kable(pkg_aut_tbl[6:nrow(pkg_aut_tbl), ])
```
...
<span style= 'color: #5F6A6A;'>I serve on the [editorial review board](https://ila.onlinelibrary.wiley.com/hub/journal/19362722/editorial-board.html) for [Reading Research Quarterly](https://ila.onlinelibrary.wiley.com/journal/19362722) and have served as an ad hoc reviewer for the following journals:</span>
