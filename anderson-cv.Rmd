...
if(PDF) {
  ...
  header <- '
  Download PDF
  <a href="https://github.com/datalorax/ach-gap-variability/raw/master/manuscript/ach-gap-variability.pdf" class="icon-block">
...
fa_git <- ifelse(
  ...
  paste0('[<i class="fas falink2 fa-github"></i>](', 
         prpas$git_repo, 
         ")"
         ),
  ""
...
```{r}
kable(pubs_tbl[seq(14, nrow(pubs_tbl)), ])
```
...
make_grant_cite <- function(title, funding_agency, award_num, role, pi,
  ...
  if(in_review) {
    budg <- paste0(". Proposed budged: $", 
                   prettyNum(budget, big.mark = ",")
                   )
  } else {
    budg <- paste0(". Total budged: $", 
                    prettyNum(budget, big.mark = ",")
                    )
  }
